...
# Default councillor list based on recent Cessnock City Council composition.
# User can edit / add / remove lines as needed for each entry.
DEFAULT_COUNCILLORS = [
    "Mayor Watton",
    "Cr Dixon",
    "Cr Dunne",
...
with st.form("add_report_form", clear_on_submit=True):
    ...
    # Allow user to customise the list of councillors for this entry
    ...
    if submitted:
        if not meeting_title.strip() or not motion_title.strip():
            ...
            # Clean votes – remove the placeholder "—"
